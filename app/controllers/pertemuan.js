...
exports.addMeet = async (req, res) => {
    const { id_pcu, id_pba, topik, tanggal, jam_mulai, jam_selesai, tempat_id } = req.body
    const post = {
        id_pcu,
        id_pba,
        topik,
            ...
            if (result) {
                ...
                Meet.findOne(
                    {where: { [Op.and]: [
                        ...
                        { [Op.or]: [
                            {jam_mulai: {[Op.between]: [req.body.jam_mulai, req.body.jam_selesai]}},
                            {jam_selesai: {[Op.between]: [req.body.jam_mulai, req.body.jam_selesai]}}
                        ]}
                    ]}
                    })
                    .then(function(result){
                        if (result) {
                            //data exist
                            res.send({status: "failed", message:"pba sedang melakukan pertemuan"})
                        } else {
                            // data not exist
                            User.findOne(
                                { where: {[Op.and]: [ {id: req.body.id_pcu}, {role: "pcu"} ]}
                                })
                                ...
                                if (result2) {
                                    ...
                                    .then(function(result2){
                                        if (result2) {
                                            ...
                                            res.send({status: "failed", message:"role id_pba bukan pba"})
                                        }
                                    })
                                    .catch((err) => {
                                        res.status(500).send({
                                    ...
                                    res.send({status: "failed", message:"role id_pcu bukan pcu"})
                                }
                            })
                            .catch((err) => {
                                res.status(500).send({
...
exports.pbaRejected = (req, res) => {
    const id = req.params.id_pertemuan;
    Meet.update({ status: "rejected", feedback_rejected: req.body.feedback_rejected }, {
        where: { id: id }
    })
        .then(num => {
            if (num == 1) {
